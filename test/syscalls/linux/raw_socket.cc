...
TEST(RawSocketTest, SetIPv6ChecksumError_ReadShortFilterFault) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveRawIPSocketCapability()));
  ...
  if (IsRunningOnGvisor() && !IsRunningWithHostinet()) {
    // TODO(https://gvisor.dev/issue/6982): This is a devation from Linux. We
...
TEST(RawSocketTest, IPv6Checksum_ValidateAndCalculate) {
  ...
  auto send = [&](const FileDescriptor& fd, uint32_t v) {
    ...
    ASSERT_THAT(sendto(fd.get(), &packet, sizeof(packet), 0 /* flags */,
  ...
  auto expect_receive = [&](const FileDescriptor& fd, uint32_t v,
    ...
    ASSERT_THAT(
        RetryEINTR(recvfrom)(fd.get(), &packet, sizeof(packet), 0 /* flags */,
