...
func (t *Task) findReparentTargetLocked() *Task {
	...
	ancestor := t.parent.tg
	for ancestor != nil {
		if ancestor.isInitLocked() {
			// We found an init process. This may return nil if the
			// init process is exiting, but we don't look any.
			return ancestor.anyNonExitingTaskLocked()
		}
		if ancestor.isChildSubreaper {
			// We found a subreaper process. Return a non-exiting
			// task if there is one, otherwise keep walking up the
			// process tree.
			if target := ancestor.anyNonExitingTaskLocked(); target != nil {
				return target
			}
		}
	}
